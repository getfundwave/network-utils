...
LAMBDA_REGION=${AWS_REGION:-'ap-south-1'}

# Auth Headers
$PYTHON_EXEC -m venv env && source env/bin/activate
pip install boto3

output=$($PYTHON_EXEC auth-header.py $ACCESS_KEY_ID $SECRET_ACCESS_KEY $SESSION_TOKEN)
...
aws lambda invoke --function-name ${CA_LAMBDA_FUNCTION_NAME} --cli-binary-format raw-in-base64-out --payload file://event.json response.json --region $LAMBDA_REGION
